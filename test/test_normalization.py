...
def test_generalize_sql():
    ...
    assert (
        remove_comments_from_sql("SELECT /* Test */ foo FROM BAR")
        == "SELECT foo FROM BAR"
    )
    ...
    assert (
        generalize_sql("SELECT foo FROM bar WHERE id IN (123,456, 789)")
...
def test_generalize_insert():
    assert (
        generalize_sql("INSERT INTO bar (foo, test) Values ( 123, 456, 789 )")
