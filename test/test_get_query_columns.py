...
def test_get_query_columns():
    ...
    assert get_query_columns("SELECT COUNT(foo), max(time_id) FROM `test_table`") == [
        "foo",
        "time_id",
    ]
    assert get_query_columns("SELECT id, foo FROM test_table WHERE id = 3") == [
        "id",
        "foo",
    ]
...
def test_get_query_columns_order_by():
    ...
    assert get_query_columns("SELECT foo FROM bar WHERE id > 20 ORDER BY id") == [
        "foo",
        "id",
    ]
    ...
    assert get_query_columns("SELECT user_id,foo FROM bar ORDER BY id LIMIT 20") == [
        "user_id",
        "foo",
        "id",
    ]
...
def test_get_query_columns_complex():
    ...
    ) == ["time_id", "pageviews", "period_id", "wiki_id"]
